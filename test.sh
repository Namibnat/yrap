...
echo "Add a project..."
curl -X POST -H "Content-Type: application/json" -d '{"title": "most test data, this time longer", "slug": "_more_test_data"}' http://localhost:5050/api/projects/add/
...
echo "Add an action"
read -s -k $'?Press any key to start...\n'
curl -X POST -H "Content-Type: application/json" -d '{"description": "buy a thing"}' http://localhost:5050/api/projects/detail/go_shop/action/
